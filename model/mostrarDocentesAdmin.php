<?php
    ...
    while($datos = $resultado->fetch_assoc()){
        if($datos['idDocentes']!=1){
            $datos['btnEditar']='<span class="btn btn-warning btn-sm text-white borde-button"  data-toggle="modal" data-target="#actualizarModal" onclick="obtenerDocentes('.$datos['idDocentes'].');" title="editar"><i class="fas fa-edit"></i></span> <span class="btn btn-danger btn-sm text-white borde-button"  data-toggle="modal" data-target="#eliminarAlumnoModal"  onclick="eliminarDocente('.$datos['idDocentes'].');" title="editar"><i class="fas fa-trash"></i></span>';
            $cont_dt['data'][]=$datos;
        }
    }
